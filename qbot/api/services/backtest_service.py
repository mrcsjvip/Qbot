...
class BacktestService:
    ...
    def create_backtest_task(self, request: BacktestCreateRequest) -> Dict[str, Any]:
        ...
        thread = threading.Thread(target=self._run_backtest, args=(task_id, backtest_params))
        thread.daemon = True
        thread.start()
    ...
    def _run_backtest(self, task_id: str, params: Dict[str, Any]):
        ...
        try:
            ...
            # 获取数据
            code = params["code"].replace(".SH", "").replace(".SZ", "")
            start_date = params["start_date"]
            end_date = params["end_date"]
            
            # 使用tushare获取数据（复用现有代码逻辑）
            ...
            # 计算胜率
            total_trades = trades_analysis.get("total", {}).get("total", {}).get("total", 0)
            won_trades = trades_analysis.get("won", {}).get("total", {}).get("total", 0)
            ...
            result = {
                ...
                "max_drawdown": abs(drawdown_analysis.get("max", {}).get("drawdown", 0)) / 100,
                "win_rate": win_rate,
